"""Tests for types extractor schema."""
import json
import sys
...
def test_types_schema_structure():
...
def test_types_schema_deserialization():
...
def test_types_schema_field_types():
    schema = get_schema_json("types")
    for field in schema["fields"]:
...
def test_types_schema_nullable_flags():
    schema = get_schema_json("types")
    for field in schema["fields"]:
        assert isinstance(field["nullable"], bool)


def test_types_schema_consistency():
